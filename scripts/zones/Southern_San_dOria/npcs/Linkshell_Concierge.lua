...
function onTrigger(player, npc)
    ...
    if (npc:getID() == 17719643) then -- the one on the right
        LinkShellConciergeEventTrigger(player, npc, 0, tpz.lsconciergetype.NEWPLAYERS)
    elseif (npc:getID() == 17719644) then -- the one on the left
        LinkShellConciergeEventTrigger(player, npc, 1, tpz.lsconciergetype.EVERYTHING)
    else
