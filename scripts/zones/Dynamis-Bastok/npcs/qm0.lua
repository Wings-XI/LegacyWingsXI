...
function onTrigger(player, npc)
    ...
    for _,v in ipairs(nearbyPlayers) do
        if v:hasKeyItem(tpz.ki.HYDRA_CORPS_EYEGLASS) == false then
            v:addKeyItem(tpz.ki.HYDRA_CORPS_EYEGLASS)
            v:messageSpecial(ID.text.KEYITEM_OBTAINED, tpz.ki.HYDRA_CORPS_EYEGLASS)
        end
    end
end
