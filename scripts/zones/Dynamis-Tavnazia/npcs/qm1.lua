...
function onTrigger(player, npc)
    ...
    for _,v in ipairs(nearbyPlayers) do
        if v:hasKeyItem(tpz.ki.DYNAMIS_TAVNAZIA_SLIVER) == false then
            v:addKeyItem(tpz.ki.DYNAMIS_TAVNAZIA_SLIVER)
            v:messageSpecial(ID.text.KEYITEM_OBTAINED, tpz.ki.DYNAMIS_TAVNAZIA_SLIVER)
        end
    end
end
