...
function imperialAgent_PotHatch(player, npc, posX, posZ, posR)
    ...
    if npc:getID() == instance:getProgress() then
        ...
        instance:setProgress(0)
         for _,v in pairs(chars) do
            npc:timer(2000, function(npc) BRUJEEL:setPos(posX, -1, posZ, posR) end)
            BRUJEEL:setStatus(tpz.status.NORMAL) BRUJEEL:entityAnimationPacket("deru")
                npc:timer(4000, function(npc) BRUJEEL:setAnimation(0) end)
            player:timer(8000, function(player) v:showText(BRUJEEL, ID.text.BRUJEEL_TEXT) end)
            ...
            player:timer(20000, function(player) v:showText(BRUJEEL, ID.text.BRUJEEL_TEXT + 6) end)
