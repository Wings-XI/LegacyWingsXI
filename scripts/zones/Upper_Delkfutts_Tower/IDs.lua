-----------------------------------
-- Area: Upper_Delkfutts_Tower
-----------------------------------
require("scripts/globals/zone")
-----------------------------------

zones = zones or {}

zones[tpz.zone.UPPER_DELKFUTTS_TOWER] =
{
    text =
    {
        THIS_ELEVATOR_GOES_DOWN       = 25,    -- This elevator goes down, but it is locked. Perhaps a key is needed to activate it.
        ITEM_CANNOT_BE_OBTAINED       = 6417,  -- You cannot obtain the <item>. Come back after sorting your inventory.
        ITEM_OBTAINED                 = 6423,  -- Obtained: <item>.
        GIL_OBTAINED                  = 6424,  -- Obtained <number> gil.
        KEYITEM_OBTAINED              = 6426,  -- Obtained key item: <keyitem>.
        FELLOW_MESSAGE_OFFSET         = 6452,  -- I'm ready. I suppose.
        CARRIED_OVER_POINTS           = 7034, -- You have carried over <number> login point[/s].
        LOGIN_CAMPAIGN_UNDERWAY       = 7035, -- The [/January/February/March/April/May/June/July/August/September/October/November/December] <number> Login Campaign is currently underway!<space>
        LOGIN_NUMBER                  = 7036, -- In celebration of your most recent login (login no. <number>), we have provided you with <number> points! You currently have a total of <number> points.
        CANNOT_ENTER_LEVEL_RESTRICTED = 7056, -- Your party is unable to participate because certain members' levels are restricted.
        FISHING_MESSAGE_OFFSET        = 7084,  -- You can't fish here.
        CONQUEST_BASE                 = 7184,  -- Tallying conquest results...
        CHEST_UNLOCKED                = 7351,  -- You unlock the chest!
        CRYSTAL_PULSATES              = 7359,  -- The crystal pulsates with an otherworldly energy...
        CRYSTAL_VACUUM                = 7360,  -- As you approach, the overwhelming force of a vacuum threatens to suck you inside the crystal...
        LIGHT_ENVELOP                 = 7362,  -- A shaft of light surges from the crystal and envolops your body!
        CRYSTAL_BECKONS               = 7363,  -- The will of the crystal beckons you...
        LIGHT_DISSIPATE               = 7365,  -- The crystalline light surrounding your body dissipates and vanishes!
        STONE_GLITTERS                = 7366,  -- A stone glitters before you...
        STONE_ILLUSION                = 7367,  -- Your hand passes through the stone as if it were but an illusion...
        PLAYER_OBTAINS_ITEM           = 7374,  -- <name> obtains <item>!
        UNABLE_TO_OBTAIN_ITEM         = 7375,  -- You were unable to obtain the item.
        PLAYER_OBTAINS_TEMP_ITEM      = 7376,  -- <name> obtains the temporary item: <item>!
        ALREADY_POSSESS_TEMP          = 7377,  -- You already possess that temporary item.
        NO_COMBINATION                = 7382,  -- You were unable to enter a combination.
        REGIME_REGISTERED             = 9460,  -- New training regime registered!
        HOMEPOINT_SET                 = 10519, -- Home point set!
    },
    mob =
    {
        ENKELADOS_PH =
        {
            [17424388] = 17424385, -- -371.586 -144.367 28.244
            [17424426] = 17424423, -- -215.194 -144.099 19.528
        },
        IXTAB_PH =
        {
            [17424472] = 17424475,
            [17424473] = 17424475,
            [17424474] = 17424475,
            [17424509] = 17424512,
            [17424510] = 17424512,
            [17424511] = 17424512,
        },
        PALLAS       = 17424444,
        ALKYONEUS    = 17424480,
    },
    npc =
    {
        TREASURE_CHEST = 17424564,
        CASKET_BASE    = 17424525,
    },
}

return zones[tpz.zone.UPPER_DELKFUTTS_TOWER]
