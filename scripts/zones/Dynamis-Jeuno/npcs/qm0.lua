...
function onTrigger(player, npc)
    ...
    for _,v in ipairs(nearbyPlayers) do
        if v:hasKeyItem(tpz.ki.HYDRA_CORPS_TACTICAL_MAP) == false then
            v:addKeyItem(tpz.ki.HYDRA_CORPS_TACTICAL_MAP)
            v:messageSpecial(ID.text.KEYITEM_OBTAINED, tpz.ki.HYDRA_CORPS_TACTICAL_MAP)
        end
    end
end
