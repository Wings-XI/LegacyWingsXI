...
function onUseAbility(player, target, ability)
    ...
    local burden = 15
    if (target:getStat(tpz.mod.INT) < target:getPet():getStat(tpz.mod.INT)) then
        burden = 20
    end

    local overload = target:addBurden(tpz.magic.ele.ICE-1, burden)
    ...
    if (overload ~= 0) then
        ...
        target:addStatusEffect(tpz.effect.ICE_MANEUVER, bonus, 0, utils.clamp(dur, 60, 300))

    end

    return tpz.effect.ICE_MANEUVER
end
