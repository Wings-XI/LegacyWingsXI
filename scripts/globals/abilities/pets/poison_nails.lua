...
function onPetAbility(target, pet, skill)
    ...
    if totaldamage > 0 then
        local resist = applyResistanceAbility(pet, target, tpz.magic.element.LIGHT, tpz.skill.ENFEEBLING_MAGIC)
        local duration = math.ceil((60 + 60 * skill:getTP()/3000) * resist * tryBuildResistance(tpz.mod.RESBUILD_POISON, target))
        if (target:getMod(tpz.mod.STATUSRES) < 100 and target:getMod(tpz.mod.POISONRES) < 100) then
            if (resist >= 0.5 and target:hasStatusEffect(tpz.effect.POISON) == false) then
                target:addStatusEffect(tpz.effect.POISON, 1, 3, duration)
            end
        end
    end
