...
zeni_fest.onSanrakuPlateTradeComplete = function(player, zeni_base)
    ...
    local zeniPercOfMax = 0 zeni_base > 5 and (zeni_base / normalizedMaxZeni) or 1
    printf("zeni %f", zeni_fest.params.MaxBonusZeni)
    -- bonus zeni is half the distance to the max
    ...
    if totalZeni > 10 or bonusZeni > 0 then
        player:PrintToPlayer(string.format("Sanraku : ZeniFest is currently active. You get %u bonus zeni and %u bonus ISP!", bonusZeni, bonusISP), 0xD)
    else
