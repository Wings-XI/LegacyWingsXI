...
-- Item Effect: This potion removes up to six negative effects.  Does not remove Curse, Doom, or Amnesia
-----------------------------------------
...
function onItemUse(target)
    ...
    if statusNum == -1 then
        return 0
    -- 6 or less effects on target
    elseif statusNum >=0 and statusNum <=5 then
        for i = 0, statusNum, 1 do
            target:delStatusEffect(has[i])
        end
    -- more than 6 effects - randomize removal
    else
        for i = 0, 5, 1 do
