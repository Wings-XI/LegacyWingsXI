...
tpz.chocobo.renterOnTrade = function(player, npc, trade, eventSucceed, eventFail)
    local zoneId = player:getZoneID()
    local info   = chocoboInfo[zoneId]

    if trade:hasItemQty(1789, 1) and zoneId == (tpz.zone.WINDURST_WOODS or tpz.zone.BASTOK_MINES or tpz.zone.SOUTHERN_SAN_DORIA) then -- Chocopass Fails for Non-Starter Cities
    ...
    elseif trade:hasItemQty(1514, 1) then -- Chocobo Ticket
        if info.past then -- Fails for Past Zones
            ...
            local currency = player:getGil()
            local price    = 0
            local duration = 1800 + (player:getMod(tpz.mod.CHOCOBO_RIDING_TIME) * 60)

            if mLvl >= 20 and player:hasKeyItem(tpz.ki.CHOCOBO_LICENSE) then
