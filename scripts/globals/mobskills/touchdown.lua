...
function onMobWeaponSkill(target, mob, skill)
    local tdMultiplier = mob:getLocalVar("savageDmgMultipliers")

    if tdMultiplier == 1 then
        tdMultiplier = 1.75
    else
        tdMultiplier = 4
    end
    ...
    local dmgmod = 1
    local info = MobMagicalMove(mob, target, skill, mob:getWeaponDmg()*tdMultiplier, tpz.magic.ele.NONE, dmgmod, TP_NO_EFFECT)
