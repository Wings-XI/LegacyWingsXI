...
function onMobWeaponSkill(target, mob, skill)
    ...
    damage = MobMagicalMove(mob, target, skill, damage, tpz.magic.ele.DARK, 1, TP_NO_EFFECT, 0)
    damage = mobAddBonuses(mob, nil, target, damage.dmg, tpz.magic.ele.DARK)
    damage = AvatarFinalAdjustments(damage, mob, skill, target, tpz.attackType.MAGICAL, tpz.damageType.DARK, MOBPARAM_WIPE_SHADOWS)
    -- Damage halved for Battaru Royale Astral Flows if target has related element KI
    if mob:getID() < 17477633 and mob:getID() > 17473591 and target:hasKeyItem(tpz.ki.DARK_SAP_CRYSTAL) then
        damage = damage / 2
    end
