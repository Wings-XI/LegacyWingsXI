...
function onSpellCast(caster, target, spell)
    ...
    spellParams.M200 = 2
    spellParams.I = 780
    spellParams.bonus = caster:getMerit(tpz.merit.TORNADO_II)*5
    --spellParams.bonusmab = caster:getMerit(tpz.merit.ANCIENT_MAGIC_ATK_BONUS)
    spellParams.AMIIburstBonus = caster:getMerit(tpz.merit.TORNADO_II)*3/100

    -- no point in making a separate function for this if the only thing they won't have in common is the name
    handleNinjutsuDebuff(caster, target, spell, 30, 15, tpz.mod.ICERES)
