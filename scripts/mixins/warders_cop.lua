...
g_mixins.warders_cop = function(mob)
    ...
    mob:addListener("COMBAT_TICK", "WARDER_CTICK", function(mob)
        ...
        if battletime - initiate >= 27 and warder <= 1 then -- warder signals to the others before using abilities
            mob:showText(mob, ID.text.BEEP_BEEP)
            mob:setLocalVar("initiate", battletime)
        elseif battletime - changeTime >= 30 and warder >= 1 then -- shortly after each will use the same ability at the same time
