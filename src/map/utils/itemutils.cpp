...
namespace itemutils
{
    ...
    ************************************************************************/
    ...
    void LoadDropList()
    {
        int32 ret = Sql_Query(SqlHandle, "SELECT dropId, itemId, dropType, itemRate, groupId, groupRate FROM mob_droplist WHERE dropid < %u ORDER BY dropId, groupId ASC;", MAX_DROPID);
