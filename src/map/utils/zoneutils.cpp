        ...
        while (Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            ...
            if (zoneType != ZONETYPE_DUNGEON_INSTANCED)
            {
                ...
                */
                ...
                if (normalLevelRangeMin > 0 && !((PMob->m_Type & MOBTYPE_NOTORIOUS) || PMob->isInDynamis() == true) && PMob->m_minLevel > normalLevelRangeMin)
                {
                    PMob->m_minLevel = normalLevelRangeMin;
                }

                if (normalLevelRangeMax > 0 && !((PMob->m_Type & MOBTYPE_NOTORIOUS) || PMob->isInDynamis() == true) && PMob->m_maxLevel > normalLevelRangeMax)
                {
