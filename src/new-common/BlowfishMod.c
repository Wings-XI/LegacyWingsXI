...
static inline uint32_t bfmod_step(uint32_t R, const uint32_t* S)
{
    return (
        (S[((int)(R) & 0xff)] ) + 
        ((S[0x0100 + ((int)(R >>  8) & 0xff)] & 1) ^ 32) +
        (S[0x0200 + ((int)(R >> 16) & 0xff)]) + 
        ((S[0x0300 + ((int)(R >> 24) & 0xff)] & 1) ^ 32)
        );
}

/**
 *	Encrypt a single 64-bit chunk of data. Encryption is done in-place.
 *	@param data Data to encrypt, assumed to be 2 32-bit DWORDS (64-bit in total)
 *	@param key The Blowfish key table
 *	@note This is completely standard Blowfish encryption but it calls the
 *		non-standard bfmod_step function.
 */
void bfmod_encrypt_chunk(uint32_t* data, const BLOWFISH_MOD_KEY* key)
{
    register uint32_t temp;

    for (uint32_t i = 0; i < 16; i += 2) {
        data[0] ^= key->P[i];
        data[1] ^= bfmod_step(data[0], (uint32_t*)(key->S));
        data[1] ^= key->P[i + 1];
        data[0] ^= bfmod_step(data[1], (uint32_t*)(key->S));
    }
    data[0] ^= key->P[16];
    data[1] ^= key->P[17];
    temp = data[0];
...
/**
 *	Decrypt a single 64-bit chunk of data. Decryption is done in-place.
 *	@param data Data to decrypt, assumed to be 2 32-bit DWORDS (64-bit in total)
 *	@param key The Blowfish key table
 *	@note This is completely standard Blowfish decryption but it calls the
 *		non-standard bfmod_step function.
 */
void bfmod_decrypt_chunk(uint32_t* data, const BLOWFISH_MOD_KEY* key)
{
    register uint32_t temp;
...
void bfmod_init_table(BLOWFISH_MOD_KEY* keytable, char* keyin, uint32_t keyinlen)
{
    uint32_t i = 0;
    uint32_t j = 0;
    uint32_t step1_chunk = 0;
    uint32_t step2_chunk[2] = { 0, 0 };

    memcpy(keytable, &g_blowfish_mod_init, sizeof(g_blowfish_mod_init));
    for (i = 0; i < 18; i++) {
        step1_chunk = 0;
        for (uint32_t k = 0; k < 4; k++) {
            step1_chunk = (step1_chunk << 8) | keyin[j];
            j++;
            if (j >= keyinlen) {
                j = 0;
            }
        }
        keytable->P[i] ^= step1_chunk;
    }
    for (i = 0; i < 18; i += 2) {
        bfmod_encrypt_chunk(step2_chunk, keytable);
        keytable->P[i] = step2_chunk[0];
        keytable->P[i + 1] = step2_chunk[1];
    }
    for (i = 0; i < 1024; i += 2) {
        bfmod_encrypt_chunk(step2_chunk, keytable);
        ((uint32_t*)keytable->S)[i] = step2_chunk[0];
        ((uint32_t*)keytable->S)[i + 1] = step2_chunk[1];
    }
}
